...
export default function FacultyContribution() {
    let contributionPublishList = useSelector(state => state.contributionReducer.contributionPublishList)
    ...
    const getContributionPublish = useCallback(
        () => dispatch(actionContribution.getContributionPublishList(curPage, limit, idFaculty)),
            ...
            <div className={`${classes.contributionList}`}>
                    ...
                    {contributionPublishList.map((contribution, index) => {
                        return <div key={index} className="col-xs-12 col-md-6 col-lg-6 col-xl-4 my-3">
                            <div className={`card text-left px-0 ${classes.cardContribution}`}>
                                ...
                                <button className={`${classes.cardDownloadButton}`}
                                    onClick={() => {
                                        setContributionDownload({
                ...
                <div className={`${contributionDownload.downloadItem.length !== 0 ? `${classes.selectedItem}` : `${classes.selectedItemHide}`}`}>
                    ...
                    <div className={`${classes.groupButton}`}>
                        <button className={`${classes.fileDown}`}
                        onClick={()=>download(connectApi)}>
                            <img src={fileDown} alt={123} />
                        </button>
                        <button className={`${classes.arrow} ${rotate ? `${classes.rotate}` : null}`}
                            onClick={animate} ><img src={arrow} alt={123} /></button>
                    </div>
                    <div className={`${rotate ? `${classes.dropdownContainer}` : `${classes.dropdownContainerHide}`}`}>
                        <h4>Drowdown Items</h4>
                        <div className={`${classes.dropdownItemContainer}`}>
                            {contributionDownload.downloadItem.map((downloadItem, index) => {
                                return <div key={index} className={`${classes.dropDownItem}`}>
